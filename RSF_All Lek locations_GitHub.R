...
LPCHdata$LEK_ID <- as.factor(LPCHdata$LEK_ID)
LPCHdata$POINT_C<-as.factor(LPCHdata$POINT_C)

LPCHdata$Data_set<- as.factor(LPCHdata$Data_set)

LPCHdata["PT_Type"] <- NA
LPCHdata$PT_Type <- ifelse(LPCHdata$POINT_TYPE == "LEK", 1,0) 

#subset leks and randoms
leks <- subset(LPCHdata, POINT_TYPE =="LEK")
randoms <- subset(LPCHdata, POINT_TYPE =="RANDOM")
save(leks, file = "leks.RData")
save(randoms, file = "randoms.RData")
